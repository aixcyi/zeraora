...
在 Django 模型中加入 `ReprMixin` 会导致生成的迁移文件（migration）依赖 zeraora，为了避免出现兼容性问题，请删除迁移文件中对 zeraora 的导入以及 `migrations.CreateModel` 中的 `bases` 参数（以下带注释的两行）。
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.CreateModel(
            ...
            bases=(zeraora.ReprMixin, models.Model),
        ),
    ]
```
