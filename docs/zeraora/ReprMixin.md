...
使用 `ReprMixin` 会导致 django 生成创建模型的迁移文件（migration）时产生对 zeraora 的依赖，虽然正常情况下不会出现任何影响，但为了避免出现兼容性问题，建议将 `migrations.CreateModel` 的 `base` 参数删除。

无论是否进行这一步操作，都不会影响后续的 migrate 和 makemigrations 操作，除非卸载了 zeraora 包。
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.CreateModel(
            ...
            bases=(zeraora.typing.ReprMixin, models.Model),  # 可以选择将此参数删除
        ),
    ]
```
