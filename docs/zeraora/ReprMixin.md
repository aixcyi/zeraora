...
在 Django 模型中加入 `ReprMixin` 会导致生成的迁移文件（migration）依赖 zeraora，为了避免出现兼容性问题，请删除迁移文件中对 zeraora 的导入以及 `migrations.CreateModel` 中的 `base` 参数（以下带注释的两行）。
...
import zeraora.typings  # 请务必将此行删除
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.CreateModel(
            ...
            # 请务必将 bases 参数删除，或剔除zeraora引入的那个值
            bases=(zeraora.typings.ReprMixin, models.Model),
        ),
    ]
```
